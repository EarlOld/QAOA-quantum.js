// copy from  https://github.com/paulfears/Graphs/blob/master/Graphs.js
class Graph {
  static contexts = {};
  ...
  static priorityQueue = function () {
    ...
    this._binarySearch = function (ranking) {
      ...
      while (end - start > 1) {
        ...
        if (ranking === this.queue[checkIndex].value) {
          ...
          while (
            checkIndex > 0 &&
            this.queue[checkIndex - 1].value == ranking
          ) {
            checkIndex -= 1;
          }
          return checkIndex;
    ...
    this.getIndex = function (lookupid) {
      ...
      while (
        this.queue[index].value == ranking &&
        index + 1 < this.queue.length
      ) {
  ...
  constructor(canvasid, fps = 60, loadContext = null) {
  ...
  drawLoop(
    bezierControlPointsX,
    bezierControlPointsY,
    text = "",
    directional = false
  ) {
    this.ctx.beginPath();
    ...
    if (text !== null && text !== "") {
      let midPointX =
        Graph.calculateMidPointOfBezierCurve(bezierControlPointsX);
      let midPointY =
        Graph.calculateMidPointOfBezierCurve(bezierControlPointsY);
  ...
  addTextOverClearBox(centerX, centerY, text) {
    ...
    this.ctx.clearRect(
      centerX - textLength / 2 - 4,
      centerY - textHeight / 2 - 8,
      textLength + 4,
      textHeight + 8
    );
    ...
    function evaluateNode(nodeid, cost) {
      ...
      for (let i = 0; i < root.children.length; i++) {
        ...
        if (visited[node.id] !== undefined) {
          //checks to see if node is visited
          continue;
        }
        let edge = this.getEdge(root.id, node.id);
        let weight = edge.weight;
        let card = new dataCard(
          node.id,
          root.id,
          cost + weight,
          Math.floor(this.getDistance(root.id, end.id)) - 1
        );
        let key = node.id;
  ...
  update() {
    ...
    if (this.connecting) {
      ...
      let xflipper = 1;
      let slope =
        (this.mousey - this.start.active.y) /
  ...
  clear() {
    ...
    // for (let edgeid in this.edges) {
    //   this.edges[edgeid].delete();
    // }
    for (let nodeid in this.objs) {
      this.objs[nodeid].delete();
    }
  }
}

Graph._edge = function (
  contextid,
  startNodeid,
  endNodeid,
  color = "#000",
  ...
  this.id = Number(
    id_time.toString() +
      Object.values(Graph.getContext(this.contextid).edges).length.toString() +
      Math.random().toString()
  ); //inline concatination
  this.slope;
  this.color = color;
  this.altColor = "#aaa";
  this.selected = false;
  ...
  this._updateValues = function (slope = null) {
    ...
    this.xstart =
      start.x +
      xflip * (Math.cos(angleOffset + Math.atan(this.slope)) * start.r);
    this.ystart =
      start.y +
      yflip * (Math.sin(angleOffset + Math.atan(this.slope)) * start.r);
    this.xend =
      end.x - xflip * (Math.cos(-angleOffset + Math.atan(this.slope)) * end.r);
    this.yend =
      end.y - yflip * (Math.sin(-angleOffset + Math.atan(this.slope)) * end.r);
...
Graph._node = function (contextid, x = false, y = false, r = false, text = "") {
  ...
  this.connect = function (n, text = "", directional = false) {
    ...
    let edge = context.edge(this.id, n.id, "#000", text, directional);
    this.edges[edge.id];
  ...
  this.inside = function (x, y, yoffset, xoffset) {
    //returns bool
